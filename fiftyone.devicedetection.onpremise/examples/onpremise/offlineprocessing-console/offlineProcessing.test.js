...
const testExample = function ({ file, portNumber }) {
  ...
  // Delete the output file from the example

  const cleanFile = `

    let deleteOutputFile = true;

    `;

  code = cleanFile + code + serverClose;
...
describe('Examples', () => {
  ...
  test('hash offline processing', (done) => {
    if (os.platform() === 'win32') {
      console.warn('Skip on Windows. Causing permission issue on output file.');
      done();
    } else {
      if (isAsync) {
        setTimeout(done, 1000);
        testExample({ file: (__dirname) + '/offlineProcessing.js' });
      } else {
        done();
      }
    }
